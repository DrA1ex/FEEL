		...
		case NumberPart:
		{
						   ...
						   if(lexemes.size() >= 2)
						   {
							   ...
							   if(lastLexeme.Type == Lexeme::Operation && (lastLexeme.LexemeData == "-" || lastLexeme.LexemeData == "+")
								   && (preLastLexeme.Type == Lexeme::Operation || preLastLexeme.Type == Lexeme::CloseBracket))
							   {
								   lexemes.pop_back();
...
std::vector<Lexeme> ExpressionImplementation::ParseExpression() const
{
	...
	SymbolType lastSymbType = Divider, currentSymbolType = Divider;
...
Token ExpressionImplementation::ConstructFromLexeme(Lexeme lexeme) const
{
	switch(lexeme.Type)
	{
